...
var rating_feedback_trial = {
    type: jsPsychHtmlButtonResponse,
    stimulus: `<div class="gold-coins-container">
        <div class="gold-coins-text">
            <h2>We appreciate that you are helping us!</h2>
            <h1 style="color: blue;></h1>
            <h1 style="color: blue;>Remember to choose the most appropriate room based on how *YOU* feel about control.</h1>
        </div>
    </div>`,
    choices: ['Continue'],
    data: {
    ...
    var trial = {
        ...
        stimulus: `
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
                ...
                `<div><h3>Press the key corresponding to the action you would like to take.</h3></div>` +
            `</div>`,
        post_trial_gap: 75,
    ...
    var trial = {
        ...
        stimulus: `
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
                initialize_agent() + 
                `<div id="action_row" class="row"></div>` +
                `<div><h1 style="color: red;">Wait until the goal appears!</h1></div>` +
            `</div>`,
        choices: "NO_KEYS",
        trial_duration: 1200,
        data: {
    ...
    var trial = {
        ...
        stimulus: `
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
                ...
                `<div id="action_row" class="row"></div>` +
                `<div><h2>Press the key corresponding to the action you want to choose for collecting the gold.</h2></div>` +
            `</div>`,
        post_trial_gap: 75,
        choices: Object.keys(allrooms[room_index]),
        data: {
        ...
        on_finish: function(data){
            ...
            if(areArraysEqual(agent_TPos, goal_TPos)){
                data.success = true;
                collected_gold = collected_gold + 1;
    ...
    var trial = {
        ...
        stimulus: `
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
                initialize_agentgoal() + 
                `<div id="action_row" class="row"></div>` +
                `<div><h3>Press the key corresponding to the action you want to choose for collecting the gold.</h3></div>` +
            `</div>`,
        post_trial_gap: 75,
        choices: Object.keys(allrooms[room_index]),
        data: {
...
//----------------------------------------------------------------------------//
// Making selection trials for rating
//----------------------------------------------------------------------------//
function make_roomselection_rating_trial(room_index1, room_index2, iftest = true) {
    var trial = {
        type: jsPsychHtmlButtonResponse,
        stimulus: `
            <div class="selection-spacer"><h2>In which room would you feel more <span style="color: blue;">in control</span>?</h2></div>
            <div class="selection-container">` +
                `<div class="selection-box">` + 
                    `<div style="display: flex; flex-direction: column; align-items: center; text-align: center">` + 
                        `<p><strong>Room 1</strong></p>` +
...
function make_showselectedroom_rating_trial() {
    var trial = {
        type: jsPsychHtmlButtonResponse,
        stimulus: `<div></div>`,
        choices: [],
        ...
        on_start: function(trial) {
            var iftimeout = jsPsych.data.get().last(1).values()[0].timeout;
            if(iftimeout){
                trial.stimulus = `<div><h1 style="color: red">You took longer than 30 seconds to respond...</h1></div>`;
                trial.choices = ['Continue']
            }else{
                trial.stimulus = `
                    <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
