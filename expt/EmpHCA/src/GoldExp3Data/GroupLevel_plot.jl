...
Path_Save = "src/GoldExp3Data/Figures/GroupLevel/"
...
subjectIDsL  = BMSdata["subjectIDsL"]
...
for i_sub = subjectIDs
    ...
    chnAll_df = temp["chnAll_df"]; chnemp_df = temp["chnemp_df"]

    chNa_df = BMSdataNa["chnA_dfs"][subjectIDs .== i_sub][1]        
    # ----------------------------------------------------------------------
    ...
    # model selection
    # ----------------------------------------------------------------------
    pmAll = BMSAll.exp_M[subjectIDs .== i_sub,:][:]
    mAll_hat = findmax(pmAll)[2]

    if i_sub ∈ subjectIDsL
        pm = BMSEmp.exp_M[subjectIDsL .== i_sub,:][:]
        m_hat = findmax(pm)[2]
        df_temp = BMSdataLHat["chnemp_dfs"][subjectIDsL .== i_sub][1]
        βs = df_temp.β
        if m_hat == 1
            ls = df_temp.l0
        elseif m_hat == 2
...
# average 
y = BMSAll.exp_r; 
dy = BMSAll.d_exp_r; x = 1:length(y)
...
figure(figsize=(5,5))
ax = subplot(1,1,1)
Y = [BMSEmp.exp_M[i,:] for 
                        i = eachindex(BMSdata["subjectIDsL"])]
...
ax.set_ylabel("P(l > 1| data)")
ax.set_xlabel("P(l < 1| data)")
tight_layout()
...
# average 
y = BMSEmp.exp_r; 
dy = BMSEmp.d_exp_r; x = 1:length(y)
fig = figure(figsize=(12,6)); ax = subplot(1,2,1)
for i = eachindex(x)
...
# choosing L subjects
# ----------------------------------------------------------------------
BMSmAllHat = BMSdata["mAll_hat"]
EmplProcessedData = ProcessedData[BMSmAllHat .== 3]
BMSmHat = [findmax(BMSEmp.exp_M[i,:])[2] for i = 1:size(BMSEmp.exp_M)[1]]
