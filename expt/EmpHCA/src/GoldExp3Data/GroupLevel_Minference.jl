...
subjectIDsload  = ExcDF.ID[ExcDF.task_outliers .== 0]
...
for i_sub = subjectIDs
    ...
    # model selection
    # ------------------------------------------------------------------------
    pmAll = [sum(chnAll_df.m .== j) for j = 1:4]; 
...
subjectIDsL = subjectIDs[LSubjects]

L_matrix_emp = Matrix(hcat([d.pm for d = ProcessedData]...)')
L_matrix_emp .= log.(max.(L_matrix_emp,0.001)); N_model = size(L_matrix_emp)[2]
L_matrix_emp = L_matrix_emp[LSubjects,:]
BMS = MCMC_BMS_Statistics(L_matrix_emp; N_Sampling = Int(1e5), 
                N_Sampling_BOR = Int(1e5), N_Chains = 50, α = 1. ./ N_model)
...
save(Path_Save * "BMS.jld2", 
        ...
        # Conditional model selection to Emp subjects sepeately
        "BMS", BMS, "mAll_hat", mAll_hat,
        "LSubjects", LSubjects, "subjectIDsL", subjectIDsL
        )
