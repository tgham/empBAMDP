...
var goldbomb_feedback_trial = {
    ...
    on_load: function(){
        const feedback2 = document.getElementById('gold_text2');
        if(collected_bomb < 2){
            feedback2.textContent = collected_bomb + " Gold Coin so far!";
        }else{
            feedback2.textContent = collected_bomb + " Gold Coins so far!";
        }
    }
};
    ...
    var trial = {
        ...
        stimulus: `
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
                ...
                `<div id="action_row" class="row"></div>` +
                `<div><h2>Press the key corresponding to the most appropriate action<br>to collect the gold.</h2></div>` +
            `</div>`,
        post_trial_gap: 75,
        choices: Object.keys(allrooms[room_index]),
        data: {
    ...
    var trial = {
        ...
        stimulus: `
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
                ...
                `<div id="action_row" class="row"></div>` +
                `<div><h1 style="color: red;">Wait until a bomb disappears!</h3></div>` +
            `</div>`,
        choices: "NO_KEYS",
        trial_duration: 1200,
        data: {
    ...
    var trial = {
        ...
        stimulus: `
            <div style="display: flex; flex-direction: column; align-items: center; text-align: center;">` + 
                ...
                `<div id="action_row" class="row"></div>` +
                `<div><h2>Press the key corresponding to the most appropriate action<br>to avoid the remaining bombs.</h2></div>` +
            `</div>`,
        post_trial_gap: 75,
        choices: Object.keys(allrooms[room_index]),
        data: {
...
function make_movegridgoalresultbomb_trial(room_index) {
    var trial = {
        ...
        on_load: function(){
            ...
            const feedback = document.getElementById('feedback');
            if(success){
                feedback.textContent = "Yayy! You avoided the bombs! :)";
                feedback.style.color = "green";
            }else{
